...
Experimental `mcpMetaByServer` is also turn-scoped. Each key is a configured MCP server name, and its object is merged into `_meta` only on model-initiated calls to that server. Server scoping is the routing boundary: an entry for an aggregate server such as `codex_apps` applies to any connector tool it routes during the turn. Codex-owned `_meta` fields cannot be replaced through this parameter. `responsesapiClientMetadata` remains Responses API request metadata and is not forwarded as MCP request `_meta`.
...
{ "method": "turn/start", "id": 30, "params": {
    ...
    // Experimental: metadata for model-initiated calls to this MCP server.
    "mcpMetaByServer": {
        "search_service": {
